...
// The contract is imported straight from the example's compiler output and
// witnesses, not via ../../contract/index.ts: that module resolves its key
// directory with node:path, which does not exist in the browser. Here the keys
// are served over HTTP instead (see publicDir in vite.config.ts).
import * as ZKLoanCreditScorer from '../../../contract/managed/zk-loan/contract/index.js';
import { witnesses, type ZkLoanPrivateState as ZKLoanCreditScorerPrivateState } from '../../../contract/witnesses';
...
export const ZKLoanProvider: React.FC<Readonly<ZKLoanProviderProps>> = ({ logger, children }) => {
  ...
  const initializeProviders = useCallback(async () => {
    ...
    // Public data provider - indexer for blockchain queries. Pass the browser's
    // WebSocket explicitly: the provider's default is `isomorphic-ws`'s named
    // `WebSocket` export, which its browser build does not have (the bundler
    // warns "WebSocket is not exported by isomorphic-ws/browser.js"). The
    // parameter is typed as Node's `ws` class; the browser's WebSocket
    // implements the standard interface graphql-ws actually uses.
