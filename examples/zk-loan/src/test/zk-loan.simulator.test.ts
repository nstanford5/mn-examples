...
// Contract unit tests against the in-memory simulator (no network needed).
// Ported from midnightntwrk/example-zkloan
// (contract/src/test/zkloan-credit-scorer.test.ts). The on-chain suite is
// zk-loan.test.ts.

import { ZkLoanSimulator } from './zk-loan.simulator.js';
...
import { type Ledger } from '../../contract/managed/zk-loan/contract/index.js';
import { type ZkLoanPrivateState } from '../../contract/witnesses.js';
import { describe, it, expect } from 'vitest';

setNetworkId('undeployed');

describe('ZkLoan contract (simulator)', () => {
  ...
  it('generates initial ledger state deterministically', () => {
    const simulator0 = new ZkLoanSimulator();
    const simulator1 = new ZkLoanSimulator();
  ...
  it('approves a Tier 1 loan and caps at the max amount', () => {
    const simulator = new ZkLoanSimulator();
    const userPin = 1234n;
  ...
  it('rejects a forged attestation that exploits an unconstrained challenge quotient', () => {
    ...
    const forgedReduction = (
      { privateState }: WitnessContext<Ledger, ZkLoanPrivateState>,
      challengeHash: bigint,
    ): [ZkLoanPrivateState, [bigint, bigint]] => {
      const q = mod(mod(challengeHash - forgedChallenge, P) * modInv(TWO_248, P), P);
      return [privateState, [q, forgedChallenge]];
    };

    const attacker = new ZkLoanSimulator({ getSchnorrReduction: forgedReduction });
    const pin = 1234n;
