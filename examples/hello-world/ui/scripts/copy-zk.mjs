// Copies the compiled hello-world ZK artifacts into public/ so the browser can
// fetch them.
//
...
// We serve them from /managed/hello-world/ (see src/midnight/providers.ts).
//
  ...
  if (!existsSync(path.join(src, dir))) {
    console.error(
      `[copy:zk] ${path.join(src, dir)} not found.\n` +
        "Compile the contract first:  yarn workspace @midnight-ntwrk/example-hello-world run compile",
    );
    process.exit(1);
  }
}
