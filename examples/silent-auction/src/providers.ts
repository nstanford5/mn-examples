...
export type SilentAuctionCircuits = keyof ImpureCircuits<unknown>;

export type SilentAuctionProviders = MidnightProviders<any>;

export function buildProviders(
  wallet: MidnightWalletProvider,
  zkConfigPath: string,
  config: NetworkConfig,
): SilentAuctionProviders {
  const zkConfigProvider = new NodeZkConfigProvider<SilentAuctionCircuits>(zkConfigPath);
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: `silent-auction-${Date.now()}`,
      // this password has requirements (capital/special chars >= 3)
      privateStoragePasswordProvider: () => 'SilentAuction-Test-Password',
