...
import type { PrivatePartyProviders } from "@/midnight/providers";
import { useMidnightProviders } from "@/providers/midnight-providers";
import { useWallet } from "@/hooks/use-wallet";

const ADDRESS_KEY_PREFIX = "private-party-ui:contract-address:";
 ...
 * joinPrivateParty in the api file).
 */
export interface DeploymentOps<T, I = void> {
  deploy: (providers: PrivatePartyProviders, input: I) => Promise<{ contract: T; address: ContractAddress }>;
  join: (providers: PrivatePartyProviders, address: ContractAddress) => Promise<T>;
}

export interface Deployment<T, I = void> {
  providers: PrivatePartyProviders | null;
