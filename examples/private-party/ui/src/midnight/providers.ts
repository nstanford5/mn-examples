...
//
// This mirrors examples/private-party/src/providers.ts (the Node harness), with
 ...
 *    examples/private-party), same as the Node test harness.
...
export async function createProviders(
  ...
  const zkConfigProvider = new FetchZkConfigProvider<PrivatePartyCircuits>(
...
async function createPrivateStateProvider(accountId: string, passphrase: string | null) {
  if (PRIVATE_STATE_STORAGE === "memory") {
    return inMemoryPrivateStateProvider<typeof PRIVATE_STATE_ID, PrivatePartyPrivateState>();
  }
  if (passphrase === null) {
  ...
  return persistentPrivateStateProvider<typeof PRIVATE_STATE_ID, PrivatePartyPrivateState>({
    accountId,
    passphrase,
  });
}
...
async function createProofProvider(
  api: ConnectedAPI,
  zkConfigProvider: FetchZkConfigProvider<PrivatePartyCircuits>,
  proving: ProvingOptions,
