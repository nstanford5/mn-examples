...
import type { TokenTransfersProviders } from "@/midnight/providers";
import { useMidnightProviders } from "@/providers/midnight-providers";
import { useWallet } from "@/hooks/use-wallet";

const ADDRESS_KEY_PREFIX = "token-transfers-ui:contract-address:";
 ...
 * joinTokenTransfers in the api file).
 */
export interface DeploymentOps<T, I = void> {
  deploy: (providers: TokenTransfersProviders, input: I) => Promise<{ contract: T; address: ContractAddress }>;
  join: (providers: TokenTransfersProviders, address: ContractAddress) => Promise<T>;
}

export interface Deployment<T, I = void> {
  providers: TokenTransfersProviders | null;
