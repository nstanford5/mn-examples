...
//
// This mirrors examples/token-transfers/src/providers.ts (the Node harness), with
...
import {
  ...
  ZK_ASSETS_PATH,
  type TokenTransfersCircuits,
  type TokenTransfersPrivateState,
} from "./contract";
import { inMemoryPrivateStateProvider, persistentPrivateStateProvider } from "./private-state";

export type TokenTransfersProviders = MidnightProviders<
  TokenTransfersCircuits,
  typeof PRIVATE_STATE_ID,
  TokenTransfersPrivateState
>;
 ...
 *    examples/token-transfers), same as the Node test harness.
...
export async function createProviders(
...
): Promise<TokenTransfersProviders> {
  ...
  const zkConfigProvider = new FetchZkConfigProvider<TokenTransfersCircuits>(
...
async function createPrivateStateProvider(accountId: string, passphrase: string | null) {
  if (PRIVATE_STATE_STORAGE === "memory") {
    return inMemoryPrivateStateProvider<typeof PRIVATE_STATE_ID, TokenTransfersPrivateState>();
  }
  if (passphrase === null) {
  ...
  return persistentPrivateStateProvider<typeof PRIVATE_STATE_ID, TokenTransfersPrivateState>({
    accountId,
    passphrase,
  });
}
...
async function createProofProvider(
  api: ConnectedAPI,
  zkConfigProvider: FetchZkConfigProvider<TokenTransfersCircuits>,
  proving: ProvingOptions,
