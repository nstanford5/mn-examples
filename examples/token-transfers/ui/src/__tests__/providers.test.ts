...
describe("createProviders", () => {
  ...
  it("serves ZK assets from /managed/token-transfers on the page origin and exposes wallet keys", async () => {
    const providers = await createProviders(fakeApi(), {
      mode: "wallet",
      proofServerUrl: "",
    });
    ...
    expect(zk.baseURL).toBe(`${window.location.origin}/managed/token-transfers`);
