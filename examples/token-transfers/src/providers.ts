...
export type TokenTransfersCircuits = keyof ImpureCircuits<unknown>;
...
export function buildProviders(
  ...
  const zkConfigProvider = new NodeZkConfigProvider<TokenTransfersCircuits>(zkConfigPath);
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: `token-transfers-${Date.now()}`,
      // this password has requirements (capital/special chars >= 3)
      privateStoragePasswordProvider: () => 'TokenTransfers-Test-Password',
