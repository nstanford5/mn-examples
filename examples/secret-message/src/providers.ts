...
export type SecretMessageCircuits = keyof ImpureCircuits<unknown>;

export type SecretMessageProviders = MidnightProviders<any>;

export function buildProviders(
  wallet: MidnightWalletProvider,
  zkConfigPath: string,
  config: NetworkConfig,
): SecretMessageProviders {
  const zkConfigProvider = new NodeZkConfigProvider<SecretMessageCircuits>(zkConfigPath);
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: `secret-message-${Date.now()}`,
      // this password has requirements (capital/special chars >= 3)
      privateStoragePasswordProvider: () => 'SecretMessage-Test-Password',
