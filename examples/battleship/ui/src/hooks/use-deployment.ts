...
import type { BattleshipProviders } from "@/midnight/providers";
import { useMidnightProviders } from "@/providers/midnight-providers";
import { useWallet } from "@/hooks/use-wallet";

const ADDRESS_KEY_PREFIX = "battleship-ui:contract-address:";
 ...
 * joinBattleship in the api file).
 */
export interface DeploymentOps<T, I = void> {
  deploy: (providers: BattleshipProviders, input: I) => Promise<{ contract: T; address: ContractAddress }>;
  join: (providers: BattleshipProviders, address: ContractAddress) => Promise<T>;
}

export interface Deployment<T, I = void> {
  providers: BattleshipProviders | null;
