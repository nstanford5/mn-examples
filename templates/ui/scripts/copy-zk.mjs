// Copies the compiled __name__ ZK artifacts into public/ so the browser can
// fetch them.
//
...
// We serve them from /managed/__MANAGED__/ (see src/midnight/providers.ts).
//
// `yarn dev` and `yarn build` run it first (Yarn 4 does not run pre* scripts).
// The source is the gitignored output of `yarn compile` in examples/__name__,
...
const src = path.resolve(here, "../../contract/managed/__MANAGED__");
const dest = path.resolve(here, "../public/managed/__MANAGED__");

for (const dir of ["keys", "zkir"]) {
  if (!existsSync(path.join(src, dir))) {
    console.error(
      `[copy:zk] ${path.join(src, dir)} not found.\n` +
        "Compile the contract first:  yarn workspace @midnight-ntwrk/example-__name__ run compile",
    );
    process.exit(1);
  }
}
