...
describe("createProviders", () => {
  ...
  it("serves ZK assets from /managed/__MANAGED__ on the page origin and exposes wallet keys", async () => {
    const providers = await createProviders(fakeApi(), {
      mode: "wallet",
      proofServerUrl: "",
    });
    ...
    expect(zk.baseURL).toBe(`${window.location.origin}/managed/__MANAGED__`);
